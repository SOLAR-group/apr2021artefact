...
/**
 * This class provides utility methods to annotate JavaParser method AST nodes with their fully qualified names. Call
 * annotateCompilationUnit() to do the grunt work.
 */
public class FullyQualifiedNames {
     ...
     * @return the fully qualified name and signature for the specified method; e.g. containing.package.TopLevelClass$InnerClass.myMethod(Foo,Bar)
     */
    ...
    /**
     * Annotate all anon inner classes with a number like javac would allocate https://github.com/javaparser/javaparser/issues/138
     *
    ...
    /**
     * This will take the supplied method name, and: if it appears to be fully qualified, return it unchanged if it
     * appears to be missing the package or top-level class name, it will add those, retrieved from the supplied
     * CompilationUnit
     * <p>
     * This will not make any assumptions about inner classes: the supplied method name must be relative to the
     * top-level class
     *
