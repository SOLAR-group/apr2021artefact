 ...
 * <p>
 * In practice SourceFile can be viewed as immutable. The only way it can be changed is via the insert/delete
 * line/statement/node or replaceNode methods, which create and return a new SourceFile as part of their signature
 */
public class SourceFileTree extends SourceFile {
    ...
    /**
     * The compilation unit is only ever made available as a copy, never a direct reference. So we can assume that it is
     * only changed by methods within SourceFile, and even then only as part of making a new SourceFile object
     * <br><br>
     * The CU nodes include, using Node.setData(), an ID that can be used to reference them. For speed, these Node-ID
     * pairs are also held in a Map. Anything extra inserted by edits does not have an ID (for now, maybe useful in
     * future?) possibly under a different key to the original IDs
     */
    ...
    private List<Integer> targetMethodExpressionIDs;
    ...
    private List<Integer> allExpressionIDs;
    ...
    /**
     * called when rebuilding CU from scratch (e.g. after reading from a file) this will update the IDs etc.
     */
    ...
    /**
     * updates the supporting lists of IDs, assuming that this.compilationUnit exists and has IDs
     * <p>
     * this is called after creating a new CU from scratch and after cloning an existing one
     */
    ...
    /**
     * Updates the lists of statement and nodeIDs in and out of the target methods
     */
    private void findStatementsAndNodes() {
        targetMethodStatementIDs = new ArrayList<>();
        targetMethodExpressionIDs = new ArrayList<>();
        targetMethodNodeIDs = new ArrayList<>();
        allStatementIDs = new ArrayList<>();
        allExpressionIDs = new ArrayList<>();
        ...
        for (Expression s : compilationUnit.getChildNodesByType(Expression.class)) {
            Integer id = s.getData(NODEKEY_ID);
            allExpressionIDs.add(id);
        }
        ...
        if (this.targetMethodRootNodes != null) {
            ...
            SortedSet<Integer> eIDs = new TreeSet<>(); // expressionIDs
            ...
            for (Node tn : this.targetMethodRootNodes) {
                ...
                for (Node n : nodesInTargetMethod) {
                    ...
                    if (id != null) { // only track nodes in the original source, i.e. those with IDs
                        ...
                        if (Expression.class.isAssignableFrom(n.getClass())) {
                            eIDs.add(id);
                        }
                    }
                }
...
//    }
    ...
    /**
     * @param statementID - this is not an index, just an ID! use {@link #getIDForStatementNumber(int)} to convert
     *                    statement numbers to IDs for use here
     * @return a modified copy of this {@link SourceFileTree}, or this {@link SourceFileTree} if the node ID does not
    ...
    /**
     * @param blockID           - this is not an index, just an ID! use {@link #getIDForBlockNumber(int)} to convert
     *                          block numbers to IDs for use here
     * @param insertionPoint    - this is an ID of a statement within the block. The IDs will have been in-order, so we
     *                          will insert before the first statement with a larger ID than the specified insertion
     *                          point This will capture situations where the insertion point has been deleted.
    ...
    /**
     * for use by any edits that change nodes in-place; a bit cleaner than removing and reinserting the node, especially
     * if removal would break syntax and thus be stopped by JavaParser (getNode() and getStatement() return copies so
     * you can't edit things that way)
     *
    ...
    /**
     * currently no checking to ensure the ID is actually for a Statement node so possibly will throw a
     * ClassCastException
     *
    ...
    /**
     * @return a list of indices into the list of statements for this source file that sit within the target method
     */
    ...
    public Expression getExpression(int ID) {
        if (this.allNodes.containsKey(ID)) {
            Expression s = (Expression) (this.allNodes.get(ID).clone());
            s.setData(NODEKEY_ID, ID);
            return s;
        } else {
            return null;
    ...
    public int getRandomExpressionID(boolean inTargetMethod, Random rng) {
        List<Integer> l = inTargetMethod ? targetMethodExpressionIDs : allExpressionIDs;
        return l.get(rng.nextInt(l.size()));
    }

    /**
     * @param destinationInTargetMethod - true unless we are targeting whole class
     * @param sourceInTargetMethod      - true for swaps (both the statements to be in target method) - false for
     *                                  replace (only the destination to be in the target method)
     * @return a Map: each key is a statement ID; each value is a list of any statements that match it; that is, the
     * space of target locations for MatchedReplaceStatement and MatchedSwapStatement (the list will contain the
     * destination statement, so if the list is size 1, there are no matching statements!)
     */
    public Map<Integer, List<Integer>> getMatchedStatementLists(boolean sourceInTargetMethod,
                                                                boolean destinationInTargetMethod) {
    ...
    /**
     * @return a list of indices into the list of nodes for this source file that sit within the target method
     */
    ...
    /**
     * Get node IDs (in whole class or just the target method) that match or extend the given class Used for matched
     * statement operators Call with e.g. Statement.class, BlockStmt.class etc.
     *
    ...
    /**
     * @return a list of indices into the list of blocks for this source file that sit within the target method
     */
     ...
     * @return the ID for the statement index, if the statements were numbered in the order returned by
     * CompilationUnit.getChildNodesByType(Statement.class) NOTE: this uses the current state! so returned IDs will be
     * different once things have been moved / deleted; might even be null if you get an inserted statement
     */
     ...
     * @return the ID for the block index, if the blocks were numbered in the order returned by
     * CompilationUnit.getChildNodesByType(BlockStmt.class) NOTE: this uses the current state! so returned IDs will be
     * different once things have been moved / deleted
     */
     ...
     * @return the statement index for the given statement ID, if the statements were numbered in the order returned by
     * CompilationUnit.getChildNodesByType(Statement.class) NOTE: this uses the current state! so returned IDs will be
     * different once things have been moved / deleted; might even be null if you get an inserted statement;
     * <p>
     ...
     * @return the block index for the given statement ID, if the blocks were numbered in the order returned by
     * CompilationUnit.getChildNodesByType(BlockStmt.class) NOTE: this uses the current state! so returned IDs will be
     * different once things have been moved / deleted
     * <p>
     * returns -1 if not ID wasn't found (probably an ID for a non-statement node)
     ...
     * @return the clone
     */
    private static CompilationUnit cloneCompilationUnitWithIDs(CompilationUnit
                                                                       cu, Map<Integer, Node> nodesToReplace) {
